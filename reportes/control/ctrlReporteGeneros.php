...
$reporte->setTitle('Reporte generos');


//Posicion del titulo
$reporte->setCellValue('A1', 'Reporte de generos');
...
$reporte->setCellValue('A2', 'ID genero');
$reporte->setCellValue('B2', 'Genero');
...
$count = 3;
$Reporte = $reporteClases->reporteGenero();


if ($Reporte != null) {
    foreach ($Reporte as $rows) {
        $reporte->setCellValue('A' . $count, $rows['id_genero']);
        $reporte->setCellValue('B' . $count, $rows['genero']);
        $reporte->setCellValue('C' . $count, $rows['cantGenero']);
        $count++;
    }
} else {
...
header('Content-Disposition: attachment;filename="Reporte-Generos-' . $date . '.xls"');
